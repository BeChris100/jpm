...
public class RuntimeEnvironment {
    ...
    public static final boolean WIN32_PRODUCTION = false;
    ...
    public static boolean usernameModified() {
        List<String> modifications = ManagementFactory.getRuntimeMXBean().getInputArguments();
    ...
    public static boolean userHomeModified() {
        List<String> modifications = ManagementFactory.getRuntimeMXBean().getInputArguments();
            ...
            if (isLinux() || isMac()) {
                try {
                    if (FileUtil.exists("/etc/hostname")) {
                        String contents = FileUtil.readString("/etc/hostname");
                        if (Utility.getLineSeparator(contents).isEmpty())
                            return contents;

                        return contents.replaceAll(Matcher.quoteReplacement(Utility.getLineSeparator(contents)), "");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
